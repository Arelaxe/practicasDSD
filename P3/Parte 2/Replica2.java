...
public class Replica2 implements Replica_I {
    ...
    public void registrarUsuario(String nombre, String pass){
        ...
        ArrayList<String> usu_rep1 = new ArrayList<>();
        ...
        try{
            if (localObject != null){ // Si la réplica existe
                usu_rep1 = localObject.getNombresUsuario(); // Buscamos los nombres de usuario de la otra réplica
                if (usu_rep1.size()>=nombres_usuario.size()){ // Si esta réplica tiene menos usuarios
    ...
    public Boolean existeUsuario(String nombre){
        ...
        try{
            if (usuarios.size()>0){ // Comprobamos si hay algún usuario registrado en esta réplica
                if (nombres_usuario.contains(nombre)) // Comprobamos si la lista de nombres contiene al usuario
                    ...
                    ArrayList<String> usu_rep1 = new ArrayList<>();
                    if (localObject!=null){ // Si la réplica existe
                        usu_rep1 = localObject.getNombresUsuario(); // Cogemos la lista de nombres de usuario de la réplica
                        if (usu_rep1.contains(nombre)){ // Comprobamos si la réplica contine ese nombre de usuario
                            existe = true;
                        }
                    }   
                }
               ...
               Replica_I localObject = buscarReplica();
                ArrayList<String> usu_rep1 = new ArrayList<>();
                if (localObject!=null){
                    usu_rep1 = localObject.getNombresUsuario();
                    if (usu_rep1.contains(nombre)){
                        existe = true;
                    }
                } 
            }
    ...
    public Boolean credencialesCorrectos (String nombre, String pass){
        ...
        try{
            ...
            System.err.println("Replica2 exception:");
            e.printStackTrace();
        }   
    ...
    public Replica_I buscarReplica(){
        ...
        try{
            ...
            localObject = (Replica_I) registry.lookup("Replica1");
        } catch (Exception e){
            System.err.println("Replica2 exception:");
            e.printStackTrace();
        }  
    ...
    public String getServerUsuario(String nombre){
        ...
        try{
            if (nombres_usuario.size()>0){ // Nos aseguramos de que tenemos algún usuario registrado
                if (nombres_usuario.contains(nombre)) // Comprobamos si tenemos ese usuario en esta réplica
                    ...
                    if (localObject!=null){ // Comprobamos que existe
                        usu_rep1 = localObject.getNombresUsuario(); // Pedimos los nombres de los usuarios de la otra réplica
                        if (usu_rep1.contains(nombre)){ // Comprobamos que está en la otra réplica
                            server = "1";
                        }
                    }   
                }
            ...
            System.err.println("getServerUsuario exception:");
            e.printStackTrace();
        }   
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            System.out.println("Replica 2 lista");
        } catch (Exception e) {
